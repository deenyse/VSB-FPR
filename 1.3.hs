...
replaceInString array col char = map (\x -> if fst x == col then char else snd x) (zip [0..] array)
...
maze originalMazePicture actions = normalisedMaze
    where
        ...
        solvedMaze = mazeSolving (replaceByCordinates originalMazePicture (getSCordinates originalMazePicture) 's' ) actions
        normalisedMaze = replaceByCordinates (replaceByCordinates solvedMaze (getSCordinates solvedMaze) '.') (getSCordinates originalMazePicture) 's'
sampleInput = ["*********",
               "* *   * *",
               "* * * * *",
               "*s* * ***",
